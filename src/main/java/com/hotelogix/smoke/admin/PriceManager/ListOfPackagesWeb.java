...
public class ListOfPackagesWeb {
	...
	public  void view_All() throws Exception
	{
		...
		//GenericMethods.selectElement(view_dropdwn, "All");
			GenericMethods.js_scroll(view_dropdwn);
			GenericMethods.clickElement(view_dropdwn);
			/*GenericMethods.selectElementByIndex(view_dropdwn, 9);
			GenericMethods.selectbyNo(view_dropdwn, 10000);*/
			GenericMethods.selectElement(view_dropdwn, "All");
	...
	public  AddActivationDateWeb NavTo_ActivationDateWeb() throws Exception
	{
		...
		for(int i=2;i<=count;i++)
		{
			...
			if(data.contains(attachPackages.c.trim()))
			{
				...
				Assert.assertEquals(data.contains(attachPackages.c.trim()), true);
			WebElement we=	GenericMethods.driver.findElement(By.xpath("//table[@class='list_viewnew']//tr["+i+"]//td[7]/a"));
			GenericMethods.javascriptScroll(we);
			we.click();
				break;
			}
